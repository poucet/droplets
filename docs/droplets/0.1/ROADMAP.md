...
|------|-----|---|---------|------------|--------|
| [ ] | P0 | 1 | Rewrite FUGUE.md to match the shipping compact schema | S | High — live demo reference |
...
| [ ] | P1 | 4 | Add `get_transport` MCP tool | S | Medium-High — lets LLM reason about timing |
| [ ] | P0 | 5 | Multi-instance end-to-end validation | M | Critical — central demo claim |
| [ ] | P0 | 6 | macOS UI verification on demo machine | S | Critical — risk mitigation |

### Phase 02: Expressive Depth (stretch, pre-demo if time permits)

| Done | Pri | # | Feature | Complexity | Impact |
|------|-----|---|---------|------------|--------|
| [ ] | P1 | 7 | Per-note expressivity inside fugues | M | High — MPE-style demo moment |

### Phase 03: Post-Demo Polish
...
Everything here is about the **LLM's view of the system** and **demo-day reliability**. No new musical primitives except `get_transport`.

#### Feature 1: Rewrite FUGUE.md
...
**Solution:** Rewrite the doc against the actual `CompactFugue` / `FugueContent` types. Lead with one tagged-layering example (bass + melody + CC sweep). Keep the constraints section (tempo changes cause drift, transport required).
...
**Solution:** Replace with a prompt that (a) describes the real `fugues: [...]` schema, (b) spells out the multi-instance workflow (`list_instances` → `set_instance_name` → use names), (c) states musical defaults (velocities 60-110, `quantize: "bar"`, tag+cancel_mode per part), (d) flags gotchas (transport must be playing, tempo drift, no `get_transport` yet — removed if Feature 4 ships).
...
**Solution:** Append a fully-worked JSON example to the `#[tool(description = ...)]` attribute: two tagged fugues (notes + cc), showing `tag`, `cancel_mode: "tag:..."`, `quantize: "bar"`, and a short CC point list with linear interpolation. Keep the existing sentences as preamble.

**Files:** [src/mcp/server.rs:768](../../../src/mcp/server.rs#L768).

---

#### Feature 4: Add `get_transport` MCP tool
...
#### Feature 5: Multi-instance end-to-end validation
...
#### Feature 6: macOS UI verification on demo machine
...
### Phase 02: Expressive Depth

#### Feature 7: Per-note expressivity inside fugues
...
**Solution:** Add two `FugueContent` variants: `PerNotePitchBend { note, points: [[beat, semitones]] }` and `PerNotePressure { note, points: [[beat, value_0_1]] }`. The fugue scheduler already handles per-note events via the one-shot dispatch path, so the audio-thread side is mostly reuse. Update the tool description and FUGUE.md once merged.

**Files:** [src/mcp/server.rs](../../../src/mcp/server.rs) (FugueContent enum + parsing), [src/fugue/types.rs](../../../src/fugue/types.rs) (if event types need expansion), [src/fugue/](../../../src/fugue/) (scheduler dispatch), [docs/FUGUE.md](../../FUGUE.md).
...
```
Phase 01 (parallel where possible):
  2 (system prompt) + 3 (queue_fugue example)  ← highest ROI, do first
  ↓
  1 (FUGUE.md rewrite)                         ← can reference the new prompt
  ↓
  4 (get_transport)
  ↓
  5 (multi-instance validation) + 6 (UI verify) ← both on demo machine
  ↓
  [If time] Phase 02 (7: per-note in fugues)
  ↓
  2026-04-21: DEMO
  ↓
  Phase 03 (8: egui migration)
```
...
| Phase | Files |
|-------|-------|
| 01 | [docs/FUGUE.md](../../FUGUE.md), [src/mcp/server.rs](../../../src/mcp/server.rs), [src/fugue/bridge.rs](../../../src/fugue/bridge.rs), [INSTALL.md](../../../INSTALL.md) |
| 02 | [src/mcp/server.rs](../../../src/mcp/server.rs), [src/fugue/types.rs](../../../src/fugue/types.rs), [src/fugue/](../../../src/fugue/) |
| 03 | New `src/ui/`, retire [frontend/](../../../frontend/) and [src/gui/](../../../src/gui/) |
